...
void referenciar_pagina(Operacao dados) {
    ...
    if (listaPaginas.quant == 0) {
        ...
        if(cache_slot != -1) {
            ...
            adicionar_pagina_SegundaChance(dados_cache);
        } else {
            // Carrega do disco para cache e depois para TLB
            ...
            listaProcessos[processo_atual].ausencia_completa++;
                ...
                adicionar_pagina_SegundaChance(dados_disco);
            }
        }
        return;
    }
    ...
    // Page fault - página não encontrada na TLB
    ...
    if(cache_slot != -1) {
        ...
        if(dados.tipo == 'M') {
            strcpy(dados_cache.mensagem, dados.mensagem);
            strcpy(cache_estruturada[cache_slot].mensagem, dados.mensagem);
        }
...
{    
    ...
    if (listaPaginas.quant > 0)
    {
        ...
        do {
            printf("  Página %d: R=%d, M=%d\n", atual->indice_pagina, atual->R, atual->M);
            atual = atual->prox;
